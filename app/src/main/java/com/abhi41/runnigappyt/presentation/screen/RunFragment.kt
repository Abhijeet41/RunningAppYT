...
import com.abhi41.runnigappyt.databinding.FragmentRunBinding
import com.abhi41.runnigappyt.presentation.MainViewModel
import com.abhi41.runnigappyt.utils.Constants.REQUEST_CODE_LOCATION_PERMISSION
...
class RunFragment : Fragment(), EasyPermissions.PermissionCallbacks {
    ...
    private fun subscribeToObservers() {
        viewModel.runs.observe(viewLifecycleOwner, { results->
            results?.let {
    ...
    private fun requestPermissions() {
        if (TrackingUtility.hasLocationPermission(requireContext())) {
            if (Build.VERSION.SDK_INT > Build.VERSION_CODES.Q) {
                if (!TrackingUtility.hasBackgroundPermission(requireContext())) {
